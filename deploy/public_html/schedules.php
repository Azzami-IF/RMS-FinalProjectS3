    ...
    <?php if ($catatan && count($catatan)): ?>
        <?php foreach ($catatan as $i => $j): ?>
            <?php
                ...
                $qty = isset($j['quantity']) ? (float)$j['quantity'] : 1.0;
                if ($qty <= 0) $qty = 1.0;
                $qtyLabel = (abs($qty - (int)$qty) < 0.00001)
                    ? (string)(int)$qty
                    : rtrim(rtrim(number_format($qty, 2, '.', ''), '0'), '.');
                $calPerServing = isset($j['calories']) ? (float)$j['calories'] : 0.0;
                $calTotal = $calPerServing * $qty;
            ?>
            <div class="list-group-item d-flex align-items-center gap-3 catatan-item catatan-selectable" data-href="<?= htmlspecialchars($detailHref) ?>" style="cursor:pointer;transition:background 0.15s;">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        <div class="small text-muted">Kalori: <?= round($calTotal) ?> kcal</div>
                        <div class="small text-muted">Porsi: <?= htmlspecialchars($qtyLabel) ?></div>
