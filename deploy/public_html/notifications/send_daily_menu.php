...
// Base URL for production deploy (used in notification links)
$baseUrl = 'https://webrms.site/';
...
// Counters for monitoring output
$sentInApp = 0;
$sentEmail = 0;
$skippedDisabled = 0;
$fallbackCount = 0;
...
foreach ($users as $u) {
    ...
    if (isset($result['hits']) && count($result['hits']) > 0) {
        ...
        $primaryUrl = $scheduleUrl !== '' ? $scheduleUrl : $recommendationFocusUrl;

        $absoluteRecommendationUrl = $baseUrl . ltrim($recommendationFocusUrl, '/');
        $absoluteRecommendationListUrl = $baseUrl . ltrim($recommendationListUrl, '/');
        $absolutePrimaryUrl = $baseUrl . ltrim($primaryUrl, '/');
        ...
        $safePrimaryUrl = htmlspecialchars($absolutePrimaryUrl, ENT_QUOTES, 'UTF-8');
        $safeRecUrl = htmlspecialchars($absoluteRecommendationUrl, ENT_QUOTES, 'UTF-8');
        $safeRecListUrl = htmlspecialchars($absoluteRecommendationListUrl, ENT_QUOTES, 'UTF-8');
        ...
        if ($inAppEnabled) {
            ...
            $notif->createNotification($userId, $title, $inAppMessage, 'menu', $absoluteRecommendationUrl);
            $sentInApp++;
        }
        ...
        if ($emailEnabled && !empty($u['email'])) {
            ...
            if ($notif->sendEmail($userId, $u['email'], $title, $emailBody, $absolutePrimaryUrl)) {
                $sentEmail++;
            }
        }
    } else {
        ...
        $absoluteFallbackUrl = $baseUrl . ltrim($fallbackUrl, '/');
        $safeFallbackUrl = htmlspecialchars($absoluteFallbackUrl, ENT_QUOTES, 'UTF-8');
        $title = 'Rekomendasi Menu Sehat Harian';
        ...
        if ($inAppEnabled) {
            $notif->createNotification($userId, $title, $inAppMessage, 'menu', $absoluteFallbackUrl);
            $sentInApp++;
        }
        ...
        if ($emailEnabled && !empty($u['email'])) {
            ...
            if ($notif->sendEmail($userId, $u['email'], $title, $emailBody, $absoluteFallbackUrl)) {
                $sentEmail++;
            }
        }
...
echo "OK send_daily_menu | users={$usersTotal} | in_app={$sentInApp} | email={$sentEmail} | skipped_disabled={$skippedDisabled} | fallback={$fallbackCount}\n";
?>