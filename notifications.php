...
function rms_notif_safe_relative_href(?string $href): ?string
{
...
function rms_render_notification_body(array $n): string
{
    ...
    if ($type === 'menu') {
        ...
        if (preg_match('/<a[^>]+href=[\"\']([^\"\']+)[\"\']/i', $messageHtml, $m)) {
            $href = rms_notif_safe_relative_href($m[1]);
        }
        ...
        $tip = null;
        if (preg_match('/Tips\s*:\s*(.+)$/is', $plain, $m)) {
            $tip = trim((string)$m[1]);
        }
        ...
        if ($img || $label || $href) {
            ...
            if ($tip) {
                echo '<div class="small text-info-emphasis mt-2"><i class="bi bi-lightbulb me-1"></i>' . htmlspecialchars($tip) . '</div>';
            }
            echo '</div>';
            echo '</div>';
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $userId) {
    ...
    if ($action === 'multi_delete' && isset($_POST['delete_ids']) && is_array($_POST['delete_ids'])) {
        $ids = array_values(array_filter(array_map('intval', $_POST['delete_ids']), fn($v) => $v > 0));
        if (count($ids)) {
        ...
        <?php foreach ($data as $n): ?>
            ...
            <div class="list-group-item <?= $cardClass ?><?= $isUnread ? ' notif-unread' : '' ?> d-flex align-items-start position-relative" <?php if ($notifId > 0): ?>id="notif-<?php echo $notifId; ?>"<?php endif; ?>>
                ...
                <div class="flex-grow-1">
                    ...
                    <?php if ($type === 'goal'): ?>
                        <div class="mt-2 small text-info-emphasis"><i class="bi bi-lightbulb me-1"></i>Tips: Evaluasi progres target Anda secara berkala untuk hasil optimal.</div>
                    <?php elseif ($type === 'reminder'): ?>
                        <div class="mt-2 small text-warning-emphasis"><i class="bi bi-info-circle me-1"></i>Jangan lupa catat asupan harian Anda!</div>
                    <?php elseif ($type === 'tip'): ?>
                        <div class="mt-2 small text-info-emphasis"><i class="bi bi-lightbulb me-1"></i>Tips nutrisi harian untuk Anda.</div>
                    <?php endif; ?>
                </div>
            </div>
        <?php endforeach; ?>
